...
class DataValidationConfig:
    ...
    base_file_path = os.path.join("airline_dataset.csv")
...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        try:
            ...
            #If there is no null value then return None
            if len(df.columns)==0:
                return None
            return df
    ...
    # We will check if base dataset and current datast having same number of columns
    def if_required_columns_exists(self, base_df:pd.DataFrame, current_df:pd.DataFrame, report_key_name:str)->bool:
        try:
